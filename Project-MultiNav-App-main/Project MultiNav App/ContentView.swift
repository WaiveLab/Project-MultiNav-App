...
struct MapScreen: View {
    ...
    @State private var routeDocument: TactileMapDocument?
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            if let document {
                TactileMapView(
                    layers: mapLayers(base: document),
    ...
    /// The visible layer stack: the base document on the bottom and, when
    /// zoomed into an intersection, its route drawn on top. Both are
    /// interactable - the library hit-tests from the top layer down, so the
    /// route wins wherever the two overlap.
    ///
    /// The layer IDs are fixed strings rather than the default fresh UUID,
    /// because the map view compares them against the layer of the element
    /// currently under the finger; a new UUID on every redraw would look like
    /// a different layer and restart the haptic mid-drag.
    private func mapLayers(base: TactileMapDocument) -> [TactileMapLayer] {
        var layers = [TactileMapLayer(id: "base", document: base, isInteractable: true)]
        if let routeDocument {
            //TactileMapLayer(document: intersectionDoc, isInteractable: true),
            layers.append(TactileMapLayer(id: "route", document: routeDocument, isInteractable: true))
        }
        return layers
    }
    ...
    private func loadOverview() {
        ...
        // Participant "0" is the debug ID that already reveals the settings
        // and map-switcher buttons; only there do the on-route elements follow
        // HapticSettings instead of the parameter set under test.
        policy.useSettingsForOnRoute = (session.participantID == "0")
        do {
    ...
    public func loadMapDocument(named name: String) {
        do {
            ...
            // The route overlay is optional: a missing file simply means this
            // intersection shows its base geometry as a single layer.
            routeDocument = try? TactileMapDocument.load(from: routeFileName(for: name), bundle: .main)
            isZoomed = true
        } catch {
    ...
    /// Filename of the route overlay for an intersection on the current
    /// overview map. The route through a given intersection differs from map
    /// to map - the same crossing is a turn in some rounds and a
    /// straight-through in others - so the overview name is part of the key.
    /// Kept in one place so the convention can be changed here alone.
    private func routeFileName(for intersectionName: String) -> String {
        "\(session.currentMapName) - \(intersectionName)"
    }
...
class OptimizedSpatialPolicy: DefaultFeedbackPolicy {
    ...
    /// When true, the on-route elements take their pattern from
    /// `HapticSettings` so the Settings screen can audition them. Left off for
    /// real participants, whose on-route feedback must stay the tested one.
    var useSettingsForOnRoute = false

    /// The pattern for an on-route element: the optimizer's parameter set
    /// during a study round, or the shared default when auditioning.
    private func onRoutePattern(for type: TactileElementType) -> HapticPattern {
        guard useSettingsForOnRoute else { return parameters.hapticPattern }
        let key: HapticPat = (type == .onRouteSidewalk) ? .onRouteSidewalk : .onRoute
        return hapticSettings.patterns[key] ?? parameters.hapticPattern
    }
    ...
    override func onEnter(element: any TactileMapElement, touchType: TouchType) {
        ...
        case .onRoute, .onRouteSidewalk:
            hapticEngine.start(pattern: onRoutePattern(for: element.elementType))
