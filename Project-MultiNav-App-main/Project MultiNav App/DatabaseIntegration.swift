        ...
        AsyncStream { continuation in
            ...
            listener = db.collection("parameterValues")
                ...
                .addSnapshotListener { snapshot, error in
                    ...
                    // A document that fails to parse is dropped, which looks
                    // exactly like no parameters arriving at all: the app waits
                    // and then quietly falls back to its defaults. Log the keys
                    // the document did carry so a schema change in the
                    // optimizer shows up instead of disappearing.
                    guard let params = ParameterSet(document: doc.data()) else {
                        print("parameterUpdates: could not parse \(doc.documentID); keys: \(doc.data().keys.sorted())")
                        return
                    }
